...
function runNewGame(): void
{
    ...
    $gameId = createGame($playerName, $secretNumber);

    gameLoop($gameId, $secretNumber);
}
 ...
 */
function gameLoop(int $gameId, string $secretNumber): void
{
    $attemptNumber = 0;
    ...
    while (true) {
        ...
        saveAttempt($gameId, $attemptNumber, $guess, $hintsString);

        if (isCorrectGuess($secretNumber, $guess)) {
            updateGameOutcome($gameId, 'угадал');
